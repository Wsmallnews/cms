<?php

return [
    'navigation_form' => [
        'type_helper' => 'If there are child navigations, the current navigation\'s link/route settings will be invalid',
        'type' => 'Navigation Type',
        'name' => 'Navigation Name',
        'name_placeholder' => 'Please enter navigation name',
        'description' => 'Description',
        'description_placeholder' => 'Please enter navigation description',
        'icon_type' => 'Navigation Icon',
        'icon_type_none' => 'No Icon',
        'icon_type_icon' => 'Icon',
        'icon_type_image' => 'Image Icon',
        'icon_fieldset' => 'Icon',
        'icon' => 'Icon',
        'icon_placeholder' => 'Please select an icon',
        'active_icon' => 'Active Icon',
        'active_icon_placeholder' => 'Please select an active icon',
        'image_icon_fieldset' => 'Image Icon',
        'image_icon' => 'Icon',
        'image_icon_uploading' => 'Uploading icon...',
        'active_image_icon' => 'Active Icon',
        'active_image_icon_uploading' => 'Uploading active icon...',
        'image_tip' => 'Please upload a square image, recommended size 200x200 pixels, non-square images will be automatically cropped',
        'page' => 'Linked Page',
        'page_placeholder' => 'Select a page (page content is managed in Pages)',
        'banner' => 'Navigation Banner',
        'banner_uploading' => 'Uploading banner...',
        'target_type' => 'Target Type',
        'target_self' => 'Current Window',
        'target_blank' => 'New Window',
        'page_content' => 'Page Content',
        'url' => 'URL',
        'url_placeholder' => 'Please enter URL',
        'route_name' => 'Route Name',
        'route_name_placeholder' => 'Please enter route name',
        'url_params' => 'Request Parameters',
        'route_param' => 'Route Parameter',
        'route_param_helper' => 'Enable if there are route parameters',
        'route_params' => 'Route Parameters',
        'route_params_helper' => 'Route parameters, leave empty if none',
        'query_param' => 'Query Parameter',
        'query_param_helper' => 'Enable if there are query parameters',
        'query_params' => 'Query Parameters',
        'query_params_helper' => 'Query parameters appended to the URL, leave empty if none',
        'custom_content' => 'Custom Content',
        'content_type' => 'Content Type',
        'content_type_placeholder' => 'Please select content type',
        'content_name' => 'Content Name',
        'content_name_placeholder' => 'Please enter content name',
        'content_options' => 'Options',
        'add_group' => 'Add Group',
        'status' => 'Navigation Status',
        'is_home' => 'Set as Homepage',
        'home_badge' => 'Home',
        'is_home_helper' => 'When checked, the type switches to Page and a page binding is required; only one node takes effect per scope',
    ],
    'navigation_type_enum' => [
        'child' => 'Child Navigation',
        'url' => 'Link',
        'route' => 'Route',
        'page' => 'Page',
        'content' => 'Content',
    ],
    'post_form' => [
        'basic_info' => 'Basic Information',
        'categories' => 'Select Category',
        'categories_placeholder' => 'Please select post category',
        'categories_empty' => 'No categories found',
        'title' => 'Title',
        'title_placeholder' => 'Please enter content title',
        'description' => 'Description',
        'description_placeholder' => 'Please enter description',
        'content_section' => 'Content',
        'main_image' => 'Main Image',
        'main_image_uploading' => 'Uploading main image...',
        'carousel_images' => 'Carousel Images',
        'carousel_images_uploading' => 'Uploading carousel images...',
        'editor_type' => 'Editor Type',
        'content_detail' => 'Content Detail',
        'content_placeholder' => 'Please enter content...',
        'content_markdown_placeholder' => 'Please enter content detail (Markdown supported)',
        'status_section' => 'Status',
        'flags' => 'Flags',
        'order' => 'Order',
        'order_placeholder' => 'Ascending order',
        'status' => 'Status',
        'scheduled_at_type' => 'Schedule Type',
        'scheduled_at' => 'Scheduled Time',
        'minutes_later' => 'Minutes Later',
        'scheduled_at_placeholder' => 'Select publish time',
        'minutes_later_placeholder' => 'Please enter minutes',
        'schedule_publish' => 'Scheduled Publish',
        'schedule_unpublish' => 'Scheduled Unpublish',
        'category_style' => 'Category Style',
        'category_style_select' => 'Select Specific Category',
        'category_style_tree' => 'Show Category Tree',
        'category_style_helper' => 'Selecting all categories will display the category tree on the left side of the page',
        'select_post' => 'Select Post',
        'select_post_placeholder' => 'Please select a post',
    ],
    'post_status' => [
        'draft' => 'Draft',
        'published' => 'Published',
        'hidden' => 'Hidden',
        'scheduled' => 'Scheduled',
    ],
    'navigation_status' => [
        'normal' => 'Normal',
        'hidden' => 'Hidden',
    ],
    'navigation_type_status' => [
        'normal' => 'Normal',
        'disabled' => 'Disabled',
    ],
    'navigation_type_form' => [
        'basic_info' => 'Basic Information',
        'name' => 'Type Name',
        'name_placeholder' => 'Please enter type name',
        'level' => 'Level',
        'level_one' => 'Level 1',
        'level_two' => 'Level 2',
        'level_three' => 'Level 3',
        'level_infinite' => 'Infinite',
        'level_helper_create' => 'Please carefully set the level you need',
        'level_helper_edit' => 'Lowering the level will not automatically delete extra child navigations, please modify carefully',
        'description' => 'Type Description',
        'description_placeholder' => 'Please enter type description',
        'status_section' => 'Status',
        'order' => 'Order',
        'order_placeholder' => 'Ascending order',
        'status' => 'Status',
    ],
    'posts_table' => [
        'title' => 'Title',
        'categories' => 'Category',
        'publisher' => 'Publisher',
        'flags' => 'Flags',
        'views' => 'Views',
        'order' => 'Order',
        'status' => 'Status',
        'published_at' => 'Published At',
        'created_at' => 'Created At',
        'updated_at' => 'Updated At',
        'search_placeholder' => 'Search title, description...',
        'flag_filter' => 'Flag',
    ],
    'navigation_types_table' => [
        'name' => 'Navigation Name',
        'level' => 'Level',
        'description' => 'Description',
        'order' => 'Order',
        'status' => 'Status',
        'created_at' => 'Created At',
        'updated_at' => 'Updated At',
        'search_placeholder' => 'Search type name, description...',
    ],
    'navigation_infolist' => [
        'type' => 'Navigation Type',
        'description' => 'Description',
        'status' => 'Status',
    ],
    'general_setting' => [
        'title' => 'General Settings',

        // Site
        'site_section' => 'Site Information',
        'site_name' => 'Site Name',
        'site_name_helper' => 'Used for browser titles and share cards; falls back to the app name when empty',
        'site_slogan' => 'Site Slogan',
        'site_slogan_helper' => 'One-line site description shown in the footer brand block',
        'logo' => 'Site Logo',
        'logo_uploading' => 'Uploading logo...',
        'logo_with_site_name' => 'Show Site Name Beside Logo',
        'logo_with_site_name_helper' => 'On: header/footer show the logo (1:1 icon mark recommended) plus the site name; Off: logo image only (for horizontal lockup images with embedded text). Without an uploaded logo, the initial-letter block plus site name is always shown',
        'favicon' => 'Favicon',
        'favicon_helper' => 'Recommended 1:1 PNG/ICO',
        'favicon_uploading' => 'Uploading favicon...',
        'homepage_banner' => 'Homepage Banner',
        'homepage_banner_uploading' => 'Uploading banner...',
        'default_og_image' => 'Default Share Image',
        'default_og_image_helper' => 'Fallback OG image when sharing to social platforms; recommended 1200x630',
        'default_og_image_uploading' => 'Uploading share image...',

        // SEO
        'seo_section' => 'SEO Settings',
        'seo_description' => 'Default Description',
        'seo_description_helper' => 'Site-wide fallback meta description; posts can override with their own',
        'analytics_code' => 'Analytics Code',
        'analytics_code_helper' => 'JS snippet (Baidu Tongji / Google Analytics) injected before </body>',

        // Contact
        'basic_info' => 'Contact Information',
        'wechat' => 'Official WeChat Account',
        'phone' => 'Contact Phone',
        'email' => 'Email',
        'address' => 'Address',
        'work_time' => 'Working Hours',
        'work_time_helper' => 'e.g. Mon-Fri 9:00-18:00',

        // Legal
        'legal_section' => 'Copyright & Filing',
        'copyright' => 'Copyright',
        'copytime' => 'Copyright Year',
        'beian_no' => 'ICP Filing Number',
        'beian_url' => 'MIIT URL',
        'beian_police_no' => 'Public Security Filing Number',
        'beian_police_url' => 'Public Security Filing URL',

        // QR codes
        'qrcode_section' => 'QR Code Upload',
        'wechat_qrcode' => 'WeChat QR Code',
        'wechat_qrcode_uploading' => 'Uploading WeChat QR code...',
        'wechat_official_qrcode' => 'Official Account QR Code',
        'wechat_official_qrcode_uploading' => 'Uploading official account QR code...',
    ],
    'flags' => [
        'hot' => 'Hot',
        'new' => 'New',
        'recommend' => 'Recommend',
        'top' => 'Top',
    ],
    'frontend' => [
        'home_empty' => 'Homepage composition is not configured yet: create a composition in "Compositions", then mark a content-type navigation node as homepage and bind it',
        'search' => 'Search',
        'search_placeholder' => 'Search title, description...',
        'search_results' => 'Search Results',
        'skip_to_content' => 'Skip to main content',
        'login' => 'Login',
        'register' => 'Register',
        'current_position' => 'Current Position',
        'all' => 'All',
        'mobile_menu' => 'Mobile Menu',
        'more' => 'More',
        'friend_links' => 'Friend Links',
        'rss_posts' => 'Posts',
        'footer_nav' => 'Footer Navigation',
        'footer_quick_nav' => 'Footer Quick Navigation',
        'quick_entry' => 'Quick Links',
        'follow_us' => 'Follow or Contact Us',
        'follow_us_desc' => 'Add WeChat or follow our official account',
        'wechat_qrcode' => 'WeChat QR Code',
        'personal_wechat' => 'Personal WeChat',
        'official_qrcode' => 'Official Account QR Code',
        'official_account' => 'Official Account',
        'contact_phone' => 'Contact Phone',
        'contact_email' => 'Contact Email',
        'contact_address' => 'Contact Address',
        'work_time' => 'Working Hours',
        'copyright_text' => 'All Rights Reserved',
        'published_at' => 'Published At',
        'views' => 'Views',
        'home' => 'Home',
        'posts_list' => 'Posts List',
        'no_posts' => 'No posts',
        'no_related_posts' => 'No related posts yet',
        'related_posts_no_source' => 'No category configured and no post component in this row to relate to',
        'post_detail' => 'Post Detail',
        'page_detail' => 'Page',
        'profile' => 'Profile',
        'current_location' => 'Current Location',
        'copyright' => 'Copyright © :copytime :copyright All Rights Reserved',
    ],
    'global_default' => [
        'navigation_group' => 'Cms Management',
    ],
    'navigation_type_resource' => [
        'model_label' => 'Navigation Type',
        'plural_model_label' => 'Navigation Types',
        'navigation_label' => 'Navigation Types',
    ],

    'post_resource' => [
        'model_label' => 'Post',
        'plural_model_label' => 'Posts',
        'navigation_label' => 'Post Management',
    ],
    'tag_resource' => [
        'model_label' => 'Tag',
        'plural_model_label' => 'Tags',
        'navigation_label' => 'Tag Management',
    ],
    'category_page' => [
        'model_label' => 'Category',
        'plural_model_label' => 'Post Categories',
        'navigation_label' => 'Post Categories',
    ],
    'general_setting_page' => [
        'navigation_label' => 'General Settings',
    ],
    'link_status' => [
        'normal' => 'Enabled',
        'hidden' => 'Hidden',
    ],
    'link_resource' => [
        'model_label' => 'Friend Link',
        'plural_model_label' => 'Friend Links',
        'navigation_label' => 'Friend Links',
    ],
    'link_form' => [
        'basic_info' => 'Basic Information',
        'name' => 'Name',
        'name_placeholder' => 'e.g. Tech Daily',
        'url' => 'URL',
        'url_placeholder' => 'https://example.com',
        'logo' => 'Logo',
        'logo_uploading' => 'Uploading logo...',
        'group_name' => 'Group',
        'group_name_placeholder' => 'e.g. Media Partners (optional)',
        'order' => 'Order',
        'order_placeholder' => 'Higher value first',
        'nofollow' => 'nofollow',
        'nofollow_helper' => 'Append rel="nofollow" to the link (no SEO weight)',
        'status' => 'Status',
    ],
    'link_table' => [
        'name' => 'Name',
        'url' => 'URL',
        'group_name' => 'Group',
        'nofollow' => 'nofollow',
        'order' => 'Order',
        'status' => 'Status',
        'created_at' => 'Created At',
        'updated_at' => 'Updated At',
        'search_placeholder' => 'Search name, URL, group...',
    ],
    'navigation_page' => [
        'model_label' => 'Navigation',
        'plural_model_label' => 'Navigations',
        'title' => 'Navigation Management',
        'navigation_label' => 'Navigation Management',
        'save_success' => 'Save Success',
        'empty_label' => 'No navigations',
        'empty_tip_label' => 'No navigations found. Please create one.',
    ],
    'footer_navigation_page' => [
        'model_label' => 'Footer Navigation',
        'plural_model_label' => 'Footer Navigation',
        'title' => 'Footer Navigation',
        'navigation_label' => 'Footer Navigation',
        'empty_label' => 'No footer navigation',
        'empty_tip_label' => 'When empty, the footer shows only brand and filing information',
    ],
    'content_type' => [
        'posts' => 'Posts List',
        'index_posts' => 'Posts Carousel',
        'post_detail' => 'Post Detail',
        'related_posts' => 'Related Posts',
        'related_posts_category_helper' => 'Falls back to the categories of the current post provided by the post component in the same row when unset',
        'related_posts_limit' => 'Limit',
    ],
    'composition_purposes' => [
        'post_sidebar' => 'Post Sidebar',
    ],
    'sidebar' => [
        'profile' => 'Profile',
        'profile_views' => 'Browse History',
        'settings_profile' => 'Edit Profile',
        'settings_password' => 'Change Password',
        'settings_two_factor' => 'Two-Factor Authentication',
    ],
    'auth' => [
        'confirm_password' => 'Confirm Password',
        'forgot_password' => 'Forgot Password',
        'login' => 'Login',
        'register' => 'Register',
        'reset_password' => 'Reset Password',
        'verify_email' => 'Verify Email',
        'index' => 'Home',
    ],
];
